...
water <- c(FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE) #Presence of water (liquid/solid/gas) in planets from the Solar System

#Construncting data frame
planets_df <- data.frame(name,type,diameter,rings) #data.frame creates a data frame from arguments
water <- data.frame(name,water) #data frame with info of presence of water 

#Merging data frame
planets_total <- merge(planets_df,water,by="name") #merge merges two data frame by selected column/row values

##Changing name of columns
names(planets_df)[names(planets_df)=="rings"] <- "Anillos" #replacing column "rings" for Anillos
names(planets_df)[4] <- "rings" #replacing column 4 for "rings"
...
subset(planets_df, subset= rings == TRUE)	#Select planets with rings. Beware of the double =!
...
#IMPORTANT: you MUST change ALL the "/" in the filepath for "\", and put the file path between " " 
...
#READING FROM EXCEL
data <- read_excel("mtcars.xlsx") #read_excel reads xls and xlsx files

##Selecting part of sheet
subset <- read_excel("mtcars.xlsx",range="A1:E6") #read_excel reads xls and xlsx files